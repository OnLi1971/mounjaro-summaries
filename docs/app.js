...
// vyber nejlepší dostupné shrnutí (preferujeme CZ)
function pickSummary(p){
  return (
    p.summary ||                // náš standard (CZ)
    p.translatedSummary ||      // dřívější název (CZ)
    p.czSummary ||              // alternativní
    p.perplexitySummary ||      // někdy takto pojmenováno
    p.description ||            // nouzově
    ''
  );
}
...
function applyFilters(posts, {q, src, sort}){
  ...
  if (q){
    ...
    rows = rows.filter(p =>
      ...
      (pickSummary(p)||'').toLowerCase().includes(qq)
    );
  }
...
function renderPage(rows, page){
  ...
  if (slice.length === 0){
    ...
    slice.forEach(p=>{
      ...
      const title = p.title || 'Shrnutí článku';
